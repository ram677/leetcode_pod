...
# Time Complexity:
# addPacket: O(1) average for set and deque operations, O(n) in worst case when forwarding a packet.
# forwardPacket: O(1) for deque and set operations.     
# getCount: O(log m) for binary search, where m is the number of packets sent to the destination.
# Space Complexity: O(k) where k is the number of unique packets in memory.