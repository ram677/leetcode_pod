...
# Time Complexity: O(n * 26^2), where n is the length of the input string word. This is because we have three nested loops, where the outer loop runs n times and the inner two loops run 26 times each.
# Space Complexity: O(n * 26^2), since we are using a 3D list f of size n * 26 * 26 to store the minimum distances.